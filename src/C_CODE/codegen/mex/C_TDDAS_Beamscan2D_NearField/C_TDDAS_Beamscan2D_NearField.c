...
void C_TDDAS_Beamscan2D_NearField(const emlrtStack *sp,
                                  ...
                                  const real_T ezAngles[49],
                                  const real_T azAngles[63],
                                  const real_T SteeringMatrix[49392],
...
#pragma omp for nowait
    for (azAngleIdx = 0; azAngleIdx < 63; azAngleIdx++) {
      if (emlrtHadParallelError)
        continue;
      if (setjmp(emlrtJBEnviron) == 0) {

        for (ezAngleIdx = 0; ezAngleIdx < 49; ezAngleIdx++) {
          memcpy(&c_C_TDDAS_Beamscan2D_NearFieldT->f0.Mat[0], &MicSignals[0],
                 25600U * sizeof(real_T));
          for (i = 0; i < 16; i++) {
            d = SteeringMatrix[(ezAngleIdx + 49 * azAngleIdx) + 3087 * i];
            if (!(d == 0.0)) {
              b_st.site = &emlrtRSI;
              pok = true;
          ...
          SpatialResponseMatrix[ezAngleIdx + 49 * azAngleIdx] = sumColumnB(a);
          if (*emlrtBreakCheckR2012bFlagVar != 0) {
            emlrtBreakCheckR2012b(&st);
          }
        }
